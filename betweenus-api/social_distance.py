...
def check_social_distance(centers, angles, idx, threshold=1.5):
    """
    ...
    for i in sorted_idxs[1:]:
        ...
        if distances[i] < threshold and i != idx:
            return True
...
def check_social_distance_advanced(xxs, zzs, angles, lookahead=0.5):
    """
    ...
    theta0 = angles[0]
    theta1 = angles[1]
    steps = np.linspace(0, lookahead, 20)
    ...
    if np.min(distances) <= max(distances[0] / 1.5, 0.5):
        return True
    return False
